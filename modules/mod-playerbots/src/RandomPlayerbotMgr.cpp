#include <set>
// Expanded starting zones and subzones (mapId, zoneId, areaId)
struct StartingArea {
    uint32 mapId;
    uint32 zoneId;
    uint32 areaId; // 0 = any area in zone
};

static const std::set<std::tuple<uint32,uint32,uint32>> StartingAreas = {
    // Human
    {0, 12, 0}, {0, 9, 0},
    // Dwarf/Gnome
    {0, 1, 0}, {0, 1, 87}, // Coldridge Valley
    // Night Elf
    {1, 141, 0}, {1, 141, 188}, // Shadowglen
    // Draenei
    {530, 3524, 0},
    // Orc/Troll
    {1, 14, 0}, {1, 14, 215}, // Valley of Trials
    // Undead
    {0, 85, 0}, {0, 85, 1519}, // Deathknell
    // Tauren
    {1, 215, 0}, {1, 215, 220}, // Red Cloud Mesa
    // Blood Elf
    {530, 3430, 0}, {530, 3430, 1519}, // Sunstrider Isle
    // Death Knight
    {609, 139, 0},
};

static bool IsInStartingArea(Player* bot) {
    uint32 mapId = bot->GetMapId();
    uint32 zoneId = bot->GetZoneId();
    uint32 areaId = bot->GetAreaId();
    for (const auto& entry : StartingAreas) {
        if (std::get<0>(entry) == mapId && std::get<1>(entry) == zoneId && (std::get<2>(entry) == 0 || std::get<2>(entry) == areaId))
            return true;
    }
    return false;
}
...
// List of starting zones (mapId, zoneId)
struct StartingZone {
    uint32 mapId;
    uint32 zoneId;
};

static const std::array<StartingZone, 9> StartingZones = {{
    {0, 12},     // Elwynn Forest (Human)
    {0, 1},      // Dun Morogh (Dwarf/Gnome)
    {1, 215},    // Teldrassil (Night Elf)
    {530, 3524}, // Azuremyst Isle (Draenei)
    {1, 14},     // Durotar (Orc/Troll)
    {0, 85},     // Tirisfal Glades (Undead)
    {1, 215},    // Mulgore (Tauren)
    {530, 3430}, // Eversong Woods (Blood Elf)
    {609, 139},  // Acherus (DK)
}};

static bool IsInStartingZone(Player* bot) {
    uint32 mapId = bot->GetMapId();
    uint32 zoneId = bot->GetZoneId();
    for (const auto& sz : StartingZones) {
        if (sz.mapId == mapId && sz.zoneId == zoneId) return true;
    }
    return false;
}
/*
...
void RandomPlayerbotMgr::UpdateAIInternal(uint32 elapsed, bool /*minimal*/)
    ...
    // Teleport bots stuck in starting areas (configurable level/timeout)
    for (auto& botPair : playerBots) {
        Player* bot = botPair.second;
        if (!bot || !bot->IsInWorld() || bot->isDead()) continue;
        // 1. Start zone stuck
        if (bot->GetLevel() > sPlayerbotAIConfig->stuckBotLevelThreshold && IsInStartingArea(bot)) {
            std::string eventKey = "startzone_timeout";
            uint32 now = time(nullptr);
            uint32 last = GetEventValue(bot, eventKey);
            if (!last) {
                SetEventValue(bot, eventKey, now, sPlayerbotAIConfig->stuckBotStartzoneTimeout);
            } else if (now - last > sPlayerbotAIConfig->stuckBotStartzoneTimeout) {
                LOG_INFO("playerbots", "Teleporting stuck bot '{}' (level {}) out of starting area (map {}, zone {}, area {}) after {}s", bot->GetName().c_str(), bot->GetLevel(), bot->GetMapId(), bot->GetZoneId(), bot->GetAreaId(), now - last);
                RandomTeleportForLevel(bot);
                SetEventValue(bot, eventKey, 0, 0);
            }
        } else {
            SetEventValue(bot, "startzone_timeout", 0, 0);
        }
        // 2. Inactivity/movement stuck
        static const std::string moveKey = "last_position";
        static const std::string stuckKey = "inactivity_timeout";
        uint32 now = time(nullptr);
        std::string pos = std::to_string(bot->GetMapId())+":"+std::to_string(int(bot->GetPositionX()))+":"+std::to_string(int(bot->GetPositionY()))+":"+std::to_string(int(bot->GetPositionZ()));
        std::string lastPos = GetEventData(bot, moveKey);
        uint32 lastMove = GetEventValue(bot, moveKey);
        if (lastPos == pos) {
            uint32 stuckLast = GetEventValue(bot, stuckKey);
            if (!stuckLast) {
                SetEventValue(bot, stuckKey, now, sPlayerbotAIConfig->stuckBotInactivityTimeout, pos);
            } else if (now - stuckLast > sPlayerbotAIConfig->stuckBotInactivityTimeout) {
                LOG_INFO("playerbots", "Teleporting bot '{}' due to inactivity (map {}, pos {}) after {}s", bot->GetName().c_str(), bot->GetMapId(), pos.c_str(), now - stuckLast);
                RandomTeleportForLevel(bot);
                SetEventValue(bot, stuckKey, 0, 0, "");
            }
        } else {
            SetEventValue(bot, moveKey, now, 0, pos);
            SetEventValue(bot, stuckKey, 0, 0, "");
        }
    }
{
...
void RandomPlayerbotMgr::RandomizeFirst(Player* bot)
{
    ...
    // Force DK bots out of the starting zone (Acherus: The Ebon Hold, map 609)
    if (bot->getClass() == CLASS_DEATH_KNIGHT && bot->GetMapId() == 609)
    {
        LOG_INFO("playerbots", "Forcing DK bot '{}' out of Acherus (map 609) after creation", bot->GetName().c_str());
        RandomTeleportForLevel(bot);
    }
}
